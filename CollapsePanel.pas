unit CollapsePanel;
(*
11.02.07 MD nach kmp umgesiedelt. Register in 'Beispiele'
*)
...
type
  ...
  protected
    ...
    procedure SetHeaderHeight(aVal: integer);
    procedure SetHeaderColor(aVal: TColor);
  public
...
begin
  ...
  Caption:='';
  fHeaderHeight := 16;
  fHeaderColor := ApplyDark(Color,100);
  ...
  HeaderPanel.Color:= fHeaderColor;
  HeaderPanel.ParentFont:=True;

  fHeaderCaption.Parent:= HeaderPanel;
  fHeaderCaption.Font.Style := [fsBold];
  fHeaderCaption.Top := (HeaderPanel.Height div 2) - (fHeaderCaption.Font.Size div 2) - 2;
  fHeaderCaption.Left := HeaderPanel.Height + 4;
  ...
  Collapser.Font.Name:='Arial';
  Collapser.Font.Size:=12;
...
begin
  if not IsCollapsed then
   begin
     IsCollapsed:=True;
     FExpandedHeight:=Height;
     { BG 080606: auskommentiert, da zu langsam
...
procedure TCollapsePanel.SetHeaderHeight(aVal: integer);
begin
  if fHeaderHeight <> aVal then
  begin
    fHeaderHeight := aVal;
    HeaderPanel.Height := fHeaderHeight;
    Collapser.Height := HeaderPanel.Height-1;
    Collapser.Width := HeaderPanel.Height-1;
    fHeaderCaption.Top := (HeaderPanel.Height div 2) - (fHeaderCaption.Font.Size div 2) - 2;
    fHeaderCaption.Left := HeaderPanel.Height + 4;
    Repaint;
  end;
...
procedure TCollapsePanel.SetHeaderColor(aVal: TColor);
begin
  if fHeaderColor <> aVal then
  begin
    fHeaderColor := ApplyDark(Color,50);
//    fHeaderColor := clRed;
//    fHeaderColor := aVal;
    HeaderPanel.Color := fHeaderColor;
    Repaint;
