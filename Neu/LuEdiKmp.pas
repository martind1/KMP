...
(* LookUpEdit: DBEdit mit Lookup-Verbindung, Indexverbindung
   ...
   unterstützt keine Filtereingaben *12
   ---------------------------------------------------------
...
{$endif}
    ...
    try
      ...
      begin
        ...
        while AFieldName <> '' do
        begin
          I := LookUpSource.NavLink.SOList.ValueIndex(AFieldName, @SOValue);
          if I < 0 then
      ...
      begin
        ...
        NewReferences.AddFmt('%s=%s',
          [LookupSource.NavLink.LongFieldName(LookUpField), SOValue]);  //AFieldValue
      end;
      ...
      begin
        ...
        begin
          NewReferences.AddFmt('%s=%s',[References.Param(I), SOValue]);  //AFieldValue
          AllNullValues := false;
        end;
