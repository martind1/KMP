...
(* Standard Druckliste
   ...
   23.04.12 md  ChildText1=BEMERKUNG: großes und breites Textfeld in ChildBand1
   17.07.14 md  psDflt.Caption:[f1;f2]=nur diese Filter listen;{f3;f4}=alle außer diesen listen
*)
interface
...
type
  TRepDflt = class(TQRepForm)
    ...
    Query1: TuQuery;
    LaBemerkung: TQRLabel;
    EdBemerkung: TQRDBText;
...
var
  ...
  IsLast: boolean;
  S1: string;
  ChildField1, NextS1: string;  //BEMERKUNG:100 [%]

  procedure AddSum(I: integer);
  var
    S, NextS: string;
  begin  //Sum1=F1;MeineSumme
...
begin {FormCreate}
  ...
  S1 := PrnSource.Bemerkung.GetString('ChildField1', '');  //ChildField1=bemerkung:100
  ChildField1 := PStrTok(S1, ':', NextS1);
  ChildText1 := ChildField1 <> '';
  ...
  ANLnk := AMu.NavLink;
  if ANLnk = nil then
    EError('%s: MuSelect oder FrmMain.DfltMultiGrid muss angegeben werden', [OwnerDotName(PrnSource)]);
  ANLnk.Query.Open;  //26.06.12
...
//    FtrText.Frame.DrawTop := false;
  ...
  begin
    ...
    if ChildText1 then
    begin
      if SameText(ChildField1, AField.Fieldname) then
        Continue;
    end;
  ...
  //Child mit Bemerkungsfeld:
  if ChildText1 then
  begin
    LaBemerkung.Enabled := true;
    EdBemerkung.Enabled := true;

    EdBemerkung.DataSet := Query1;
    EdBemerkung.Datafield := ChildField1;
    EdBemerkung.Width := Line.Width - EdBemerkung.Left - 2;

    AField := Query1.FindField(ChildField1);
    LaBemerkung.Caption := AField.DisplayLabel + ':';
  end;
...
var
  ...
  S1: string;
begin
  Value := PrnSource.Caption;  //übernehmen

  DoIt := false;
  L := TValueList.Create;
  L1 := TValueList.Create;
  L2 := TValueList.Create;
  try
    if EndsWith(Value, ']') then
    begin   //hallo[F1;F2] -> L:12;P:6;C:5
      P := Pos('[', Value);
      L1.AddTokens(Copy(Value, P+1, Length(Value) - P - 1), ';');
      S := Value;
      Delete(S, P, Maxint);
      DoIt := true;
    end else
    ...
    begin   //hallo{F1;F2} -> L:12;P:6;C:5
      P := Pos('{', Value);
      L2.AddTokens(Copy(Value, P+1, Length(Value) - P - 1), ';');
      S := Value;
      Delete(S, P, Maxint);
      DoIt := true;
    end else
    if EndsWith(Value, ';') then
    begin
      S := Copy(Value, 1, Length(Value) - 1);
      DoIt := true;
    end;

    if DoIt then  //S ist definiert und enthält konstanten + dynamischen Anteil
    try
      ...
      begin
        S1 := L.Param(I);
        if (L1.Count > 0) and (L1.IndexOf(S1) < 0) then  //IndexOf ist *nicht* case-sensitive
          Continue;
        if L2.IndexOf(S1) >= 0 then  //Ausschluß
          Continue;
...
begin
  if aMu = nil then
    Exit;
  ANLnk := AMu.NavLink;
  if (ANLnk = nil) or (ANLnk.DataSet = nil) then
    Exit;
  if ANLnk.DataSet is TUQuery then
  begin
    Query1.Params.AssignValues(TUQuery(ANLnk.DataSet).Params);
