unit DelphiStr;
(* ResourceStrings von Delphi übersetzen
*)
interface
...
uses
  ...
  Contnrs, ActnList,
  DBLogDlg, DBPWDlg, Clipbrd, VDBConsts, Dialogs, Math, Themes,
  Windows, SysUtils, consts, bdeconst, comstrs, dbconsts, IniFiles, Classes,
  Forms,
...
begin
  ...
  newStr := StrNew(PChar(GNavigator.TranslateStr(self, Trim(LoadResString(rs)))));
...
//26.12.11  HookResourceString(@SCannotCreateName);
...
//  HookResourceString(@SAccessViolation);
  ...
  HookResourceString(@SOSError);
  HookResourceString(@SUnkOSError);
...
var
  ...
  begin
    ...
    if P > 0 then
      S := Trim(copy(S, 1, P-1));
    result := Trim(copy(S, 2, length(S) - 3));
