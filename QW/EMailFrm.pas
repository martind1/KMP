...
(* EMails erzeugen (QW)
   ...
   28.07.14 md  SaveBestaetigung, SaveToDB
   01.08.14 md  Save:Subjekt auf 255 begrenzt
*)
interface
...
type
  ...
  private
    ...
    procedure SaveToDB(Success: boolean; WerkNr, LfskNr: string);
  protected
  ...
  public
    ...
    class procedure SmsHandy(WerkNr, LfskNr, Handynr, StoerText: string; aText: string = '');
    class procedure SmsStoerung(WerkNr, LfskNr, StoerText: string);
    class function SendEMailTo(aSubject, aAttachFn: string; aText, aTo: TStrings;
      WerkNr, LfskNr: string): boolean;
    class procedure SaveBestaetigung(Success: boolean; EmailTo, Subject,
      EmailText, Attach, WerkNr, LfskNr: string); static;
  end;
...
// 28.07.14 Kurze geändert vom´n emai nach EMAIL. Achtung:QUPE/SDBL/QUPP
begin
  if FrmEmail = nil then
  begin
    GNavigator.StartFormShow(Application, 'EMAIL', wsMinimized);
  end;
end;
...
procedure TFrmEMail.SaveToDB(Success: boolean; WerkNr, LfskNr: string);
var
  Status: string;
begin
{$IF Defined(QUVA) or Defined(QSBT)}
  ...
  edWerkNr.Text := WerkNr;
  edLfskNr.Text := LfskNr;
  edStatus.Text := FrmMain.AswEmailStatus.FindValue(Status);
  Prot0('%s SaveToDB(%s,%s)', [Kurz, WerkNr, LfskNr]);
  ...
  begin
    ...
    ParamByName('EMAI_SUBJECT').AsString := copy(edEMailSubject.Text, 1, 255);  //ORA-12899: Wert zu groß
    ParamByName('EMAI_TEXT').AsString := MeEmailText.Lines.Text;
    ParamByName('EMAI_ATTACHMENTS').AsString := edEMailAttach.Text;
    ...
    ParamByName('ERFASST_VON').AsString := SysParam.UserName + '/' +
      OnlyFilename(ExtractFileName(Application.ExeName)) + '@' + CompName;
...
class procedure TFrmEMail.SaveBestaetigung(Success: boolean;
  EmailTo, Subject, EmailText, Attach, WerkNr, LfskNr: string);
// public für beliebige Nachricht speichern (ohne Mail/Sms)
begin
  StartClient;
  with FrmEMail do
  begin
    LbEmailTo.Items.CommaText := EmailTo;
    edEMailSubject.Text := Subject;
    MeEmailText.Lines.Text := EmailText;
    edEMailAttach.Text := Attach;
    SaveToDB(Success, WerkNr, LfskNr);
  end;
end;

class procedure TFrmEMail.SmsHandy(WerkNr, LfskNr, Handynr, StoerText: string; aText: string = '');
//QSBT HR
var
  S: string;
  OldWerk: string;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          edEMailAttach.Text := '';
          btnSend.Click;
          SaveToDB(EmailSendKmp.SendOK, WerkNr, LfskNr); // Aufzeichnung aller Nachrichten
        end;
      end;
    end else
    if not btnAutomatik.Down then
...
class procedure TFrmEMail.SmsStoerung(WerkNr, LfskNr, StoerText: string);
//QSBT
begin
  if FrmEMAIL <> nil then with FrmEMAIL do
  begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        btnSend.Click;
        Result := EmailSendKmp.SendOK;
        SaveToDB(Result, WerkNr, LfskNr); // Aufzeichnung aller Nachrichten
      end;
    end;
  end else
