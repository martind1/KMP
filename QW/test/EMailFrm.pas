...
type
  ...
  public
    ...
    class function SendEMailTo(aSubject: string; aText, aTo: TStrings): boolean;
  end;
...
class function TFrmEMail.SendEMailTo(aSubject: string; aText,
...
begin
  ...
  begin                 {Datenbankeinstellungen überschreiben INI-Einstellungen}
    ...
    EdPollInterval.Text := IntToStr(Nav.PollInterval);
  end;
...
begin
  ...
  try
    ...
    edSMTPPassword.Text := DecryptPassw(IniKmp.ReadString(Kurz, edSMTPPassword.Name, edSMTPPassword.Text));
...
begin
  ...
  if not PrgParam.QsbtAKW then  //Akw: Emeilgruppen (EMGR)
  ...
  IniKmp.WriteString(Kurz, edSMTPPassword.Name, EncryptPassw(edSMTPPassword.Text, 20));
...
//  with IdMsgSave do
//  try
//    ForceDirectories(ValidDir(EdOutBox.Text));
//    FN := CreateUniqueFileName(
//      ValidDir(EdOutBox.Text) + Format('%s_#Y#M#D#H#N#S_#C.EML', [CompName]));
//    DeleteFile(FN);
//    Clear;
//    Flags := [mfDraft];
//    AddHeader('X-Unsent: 1');           {OE}
//    AddHeader('X-Filename: ' + FN);
//    Attach := Trim(PStrTok(edEMailAttach.Text, ',;', NextS));
//    while Attach <> '' do
//    begin
//      {MessageParts is a TIdMessagePartsList used to store the TIdMessagePart components that make up the message}
//      TIdAttachment.Create(MessageParts, Attach);
//      Attach := Trim(PStrTok('', ',;', NextS));
//    end;
//
//    Body.Assign(MeEmailText.Lines);
//    if EdUserEmail.Text <> '' then
//    begin
//      From.Address := EdUserEmail.Text;
//      From.Name := Application.Title;
//      ReplyTo.EMailAddresses := EdUserEmail.Text;
//    end;
//    Recipients.EMailAddresses := LbEmailTo.Items.CommaText;  { To: header }
//    Subject := edEMailSubject.Text; { Subject: header }
...
//    SaveToFile(FN);
//  except on E:Exception do
//    EProt(self, E, 'Fehler bei btnSendClick(%s)', [FN]);
//  end;
end;
      ...
      try
        ...
        begin
          try
            ...
            MeEmailText.Lines.Assign(FieldByName('EMAI_TEXT')); //ist blob
...
procedure TFrmEMail.BtnPollClick(Sender: TObject);
begin
  PollKmp.Sleep(NavPoll, Nav, 0);
end;
