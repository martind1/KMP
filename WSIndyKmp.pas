unit WSIndyKmp;
(* Winsocket-DDE mit Indy-Komponenten (statt ScktComp)
   ...
   26.09.09        mit Indy (Bsp: Lawa\LIPC) <------- abgebrochen
*)
interface
...
implementation
uses
  Prots, Poll_Kmp, GNav_Kmp, Err__Kmp, MMGrDlg;
...
begin
  ...
  if Level in [wmConnection, wmError] then
    OldProt.Clear;
  S := Format(Fmt, Args);
  for I := 0 to OldProt.Count - 1 do
    if OldProt[I] = S then
      Exit;
  while OldProt.Count >= OldCount do
    OldProt.Delete(0);
  OldProt.Add(S);
...
begin
  ...
  begin
    ...
    Port := StrToIntDef(copy(Value, P1 + 1, 100), FPort);
  end else
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        ASocket.SendText(S + EolStr);  //ergibt Anzahl gesendeter Bytes
      end else
      begin
...
begin      //wird bei Connect vom System aufgerufen
  ...
  begin
    ...
    Socket.SendText('HELLO=' + OwnerDotName(self) + EolStr);  //ergibt Anzahl gesendeter Bytes
  end;
end;
...
begin
  ...
  *)
  ...
  begin
    if S <> '' then  //nur Inhalte übertragen
    begin
      XProt(wmData, 'Cli:Send(%s)', [S]);  //ProtMode - swe 15.05.04
      FClientSocket.Socket.SendText(S + EolStr);  //ergibt Anzahl gesendeter Bytes
    end;
  end else
  begin
    FSendPending := true; //senden warten bis in Poll geöffnet
