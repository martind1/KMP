...
//  LResources, LCLType,
//{$ENDIF}
  CREditorFrm, DAConnectionEditorFrm, MemUtils, DBAccess, Uni, SpecificOptionsFrame;

type
  TFrmConnectionEditor = class(TFrmDAConnectionEditor)
    lbProvider: TLabel;
  ...
  protected
    FOptionsFrame: TFrSpecificOptions;
...
function TFrmConnectionEditor.GetConnection: TUniConnection;
begin
  Result := FConnection as TUniConnection;
end;

procedure TFrmConnectionEditor.SetConnection(Value: TUniConnection);
begin
  FConnection := Value;
end;

procedure TFrmConnectionEditor.DoInit;
var
  Index: integer;
{$IFDEF MSWINDOWS}
{$IFNDEF CLR}
...
{$ENDIF}
  ...
  FOptionsFrame := TFrSpecificOptions.Create(Self);
...
procedure TFrmConnectionEditor.ConnToControls;
begin
...
procedure TFrmConnectionEditor.ControlsToConn;
begin
...
procedure TFrmConnectionEditor.ShowState(Yellow: boolean = False);
begin
  inherited;

end;

procedure TFrmConnectionEditor.SetupForSpecificOptions;
begin
...
procedure TFrmConnectionEditor.EnableMacroButtons;
var
  i: integer;
begin
...
procedure TFrmConnectionEditor.FillInfo;
begin
...
{$IFDEF MSWINDOWS}
procedure TFrmConnectionEditor.AddServerToList;
begin
  inherited;
...
procedure TFrmConnectionEditor.edProviderChange(Sender: TObject);
begin
  try
...
procedure TFrmConnectionEditor.edDatabaseExit(Sender: TObject);
begin
  try
...
procedure TFrmConnectionEditor.edDatabaseDropDown(Sender: TObject);
var
...
procedure TFrmConnectionEditor.edDatabaseKeyDown(Sender: TObject;
...
procedure TFrmConnectionEditor.edPortExit(Sender: TObject);
begin
  if FInDoInit then
    Exit;
...
procedure TFrmConnectionEditor.edPortChange(Sender: TObject);
begin
...
{$IFNDEF CLR}
procedure TFrmConnectionEditor.ReplaceEdit(var Edit: TWinControl);
type
...
procedure TFrmConnectionEditor.SpinEditKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if Key = 13 then
    btConnectClick(self);
end;
...
procedure TFrmConnectionEditor.PageControlChange(Sender: TObject);
begin
  inherited;
...
procedure TFrmConnectionEditor.FormCreate(Sender: TObject);
begin
  inherited;
  FrmConnectionEditor := self;
end;

procedure TFrmConnectionEditor.FormDestroy(Sender: TObject);
begin
  FrmConnectionEditor := nil;
  inherited;
end;

procedure TFrmConnectionEditor.FormShow(Sender: TObject);
begin
  inherited;
  TDBAccessUtils.SetLockLoginPrompt(Connection, True);
end;

procedure TFrmConnectionEditor.btAddMacroClick(Sender: TObject);
var
...
procedure TFrmConnectionEditor.btReplaceMacroClick(Sender: TObject);
var
  Name, Value, Cond: string;
  Macro: TUniMacro;
  i: integer;
...
procedure TFrmConnectionEditor.btDeleteMacroClick(Sender: TObject);
var
  i: integer;
begin
  i := sgMacros.Selection.Top;
...
procedure TFrmConnectionEditor.btClearAllMacrosClick(Sender: TObject);
begin
...
procedure TFrmConnectionEditor.UpBtnClick(Sender: TObject);
var
  i: integer;
  Sel: TGridRect;
begin
...
procedure TFrmConnectionEditor.DownBtnClick(Sender: TObject);
var
  i: integer;
  Sel: TGridRect;
begin
...
procedure TFrmConnectionEditor.sgMacrosDrawCell(Sender: TObject; ACol,
...
procedure TFrmConnectionEditor.sgMacrosSelectCell(Sender: TObject;
...
procedure TFrmConnectionEditor.edMacroConditionDropDown(
  Sender: TObject);
var
  List: TStringList;
  i: integer;
