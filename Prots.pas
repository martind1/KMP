...
12.08.14 md  ProcAssign??? entfernt; jetzt TuStoredProc Methode
   -------------------------------------
...
type
  ...
  public
    ...
    ColorActiveRow: boolean;   {true:Aktive Zeile hellblau färben bei MultiGrid}
...
function UqS(Name: string): TStrings; //ergibt Strings für Unique-Prots
...
procedure Prot0_Unique(N: integer; L: TStrings;
  const Fmt: string; const Args: array of const); {Prot0 aber nur wenn Text <> der letzten N Texte }
procedure ProtL_Unique(N: integer; L: TStrings;
  const Fmt: string; const Args: array of const);
...
//12.08.14 entfernt, siehe TUStoredProc
...
begin
  ...
  UniqueStringsList := TStringList.Create;
  UniqueStringsList.CaseSensitive := False;
  if Prot <> nil then
  begin
...
function TProt.GetUniqueStrings(Name: string): TStrings;
var
  I: integer;
begin
  I := UniqueStringsList.IndexOf(Name);
  if I < 0 then
  begin
    I := UniqueStringsList.AddObject(Name, TStringList.Create);
  end;
  Result := TStrings(UniqueStringsList.Objects[I]);
end;
...
function UqS(Name: string): TStrings;
//ergibt Strings für Unique-Prots
begin
  Result := Prot.GetUniqueStrings(Name);
end;
...
procedure ProtL_Unique(N: integer; L: TStrings;
  const Fmt: string; const Args: array of const);
{in Protokolldatei schreiben (wenn neu) und Statuszeile (immer) }
begin
  SMess(Fmt, Args);
  Prot0_Unique(N, L, Fmt, Args);
end;
...
07.07.14 Trenner ist gesamt eindeutig. (Blank+X sucht genau nach diesem Tenner)
}
var
  S, NextS: string;
...
//          ProtA('%d:%s=%s', [I, aProc.Params[I].Name, aProc.Params[I].AsString]);
//      except
//        //Fehler bei Debug
//      end;
//    end;
...
// erweiterte Funktionalität von TuMetadata
begin
...
begin
  ...
  try
    ...
    AMeta.UMetaDataKind := umdkIndexes;
    AMeta.TableName := ATblName;
    //ATblName  		// 'Indexinfo: Öffne %s'
    ProtL(SProts_010, [AMeta.SchemaName + '.' + AMeta.OnlyTableName + '@' + AMeta.SchemaParam]);
    AMeta.Open;
    if AMeta.EOF then
    begin  //nehme Synonym an
      AMeta.SynonymName := ATblName;  //wandelt Synonym nach Tablename
      if AMeta.TableName <> ATblName then
        ProtL(SProts_010, [AMeta.SchemaName + '.' + AMeta.OnlyTableName + '@' + AMeta.SchemaParam]);
      AMeta.Open;
    end;
    ...
    begin
      ...
      begin
        ...
        if (PKeyName = '') or SameText(S1, AMeta.OnlyTableName + '_ID') or
    ...
    AMeta.UMetaDataKind := umdkIndexColumns;
    ...
    AMeta.UMetaDataKind := umdkConstraints;
