...
uses
  ...
  Menus, VirtualTable, UMem_Kmp, JvBaseDlg, JvBrowseFolder, UQue_Kmp;
...
type
  TFrmDATNED = class(TqForm)
    ...
    procedure BtnDatnVerClick(Sender: TObject);
...
//unidac    with TblLokal.IndexDefs.AddIndexDef do
//    begin
//      Name := '';
//      Fields := 'FILENAME';
//      Options := [ixPrimary];
...
begin
  S := Ordner;
  if WMessInput(self.ShortCaption, BtnNewOrdner.Hint, S) then
  begin
    AddOrdner(S);
  end;
end;
...
begin
  ...
  Ordner := aOrdner;  //cob und Filelist in Gui. Immer ab 17.11.13
end;
...
//  end;
  ...
  for I := L.Count - 1 downto 0 do  //Sortieren A->Z
  begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if FI.IsLokal then
        begin
          if FI.CompareLokalDT < 0 then  //FI.LokalDT < FI.DbDT
            clFont := clGreen else
          if FI.CompareLokalDT > 0 then  //FI.LokalDT > FI.DbDT
            clFont := clRed;
        end else
...
procedure TFrmDATNED.MiRefreshClick(Sender: TObject);
begin
  Datn.ClearFileList(cobOrdner.Text);
  cobOrdnerChange(Sender);
end;
...
procedure TFrmDATNED.BtnDatnVerClick(Sender: TObject);
var
  InternalName, FileVersion: string;
begin
  GetFileInfo(InternalName, FileVersion, Application.Exename);
  AddOrdner(DatnApps + InternalName);

  if TDlgDatnVer.Execute then
  begin
    Ordner := DatnApps + InternalName;   //Refresh
  end;
end;
