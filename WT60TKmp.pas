unit wt60tkmp;
(* Fahrzeugwaage Widra WT 60 Tolerant
   17.08.00 TS Parameter Tolerant
               Wiederholung ausgeschaltet => Fehlermeldung an Benutzer
   09.02.01 MD die alte WT60 muß bleiben wegen Solucom
*)
...
type
  TWT60TKmp = class(TFaWaKmp)
  private
    { Private-Deklarationen }
    FTolerant: boolean;        {nur Auwertung Stillstand}
  ...
  published
    ...
    property Tolerant: boolean read FTolerant write FTolerant;
...
procedure TWT60TKmp.BuildDescription;
begin
...
constructor TWT60TKmp.Create(AOwner: TComponent);
begin
...
procedure TWT60TKmp.Loaded;
begin
  inherited Loaded;
  BuildDescription;
end;

destructor TWT60TKmp.Destroy;
begin
  StrDispose(Antwort);
  Antwort := nil;
  inherited Destroy;
...
procedure TWT60TKmp.DoAntwort(Sender: TObject; Tel_Id: Longint);
var
...
begin
  ...
  try                                {Protdruck=false: von AFB ->in Rollspeicher}
    ...
    try
      ...
      Druckerstörung (kein Papier):
      WAAGEPCX01KPAPRB1 }

      Stoer := ' ';

      WaStr := String(StrLPas(Empf + 10, 3));
      if WaStr = 'KPA' then
      begin
        FaWaStatus := FaWaStatus + [fwsDruckerstoerung];
      end;

      {nur wenn sinnvolle Werte vorhanden}
      if L > 32 then
      begin
        if Empf[60] <> 'G' then
        begin
        ...
        if Empf[62] <> 'N' then
        begin
          FaWaStatus := FaWaStatus + [fwsUeberlast];
          Stoer := Empf[62];          {o = Überlast}
        end;

        If Not(AWG27) Then Begin                                            {JM01}
          if Empf[63] <> 'N' then
          begin
            FaWaStatus := FaWaStatus + [fwsUnterlast];
          ...
          if Empf[67] <> 'N' then
          begin
            FaWaStatus := FaWaStatus + [fwsNullbereichsfehler];
          ...
          {mit alten Fehlermeldungen ist das schon Tolerant
          FaWaStatus := FaWaStatus - [fwsBereichsfehler]; }

          {Wenn Fehlertolerant, dann diese Fehler nicht melden}
          if Tolerant then begin
            if fwsBereichsfehler in FaWaStatus then
              FaWaStatus := FaWaStatus + [fwsBereichsfehler];
            if fwsUeberlast in FaWaStatus then
              FaWaStatus := FaWaStatus - [fwsUeberlast];
            if fwsUnterlast in FaWaStatus then
              FaWaStatus := FaWaStatus - [fwsUnterlast];
            if fwsNullbereichsfehler in FaWaStatus then
              FaWaStatus := FaWaStatus - [fwsNullbereichsfehler];
          end;
  ...
  {>JM01}
        Else Begin
          WiTyp := 'B';
          GewStr := String(StrLPas(Empf + 47, 10));
        End;
  {<JM01}

        StrToGewicht(StrCgeChar(GewStr, '.', #0 {DecimalSeparator}), Gewicht);

        {hier nicht nötig
        Gewicht := Gewicht / 1000; }

        Display := Format('%.*f', [NachK, Gewicht]);
        S1 := String(StrLPas(Empf + 34, 14));                 {LfNr: ergint immer 0}
    ...
    begin
      ...
      begin
        {Druckerstörungen abfangen}
        if (fwsDruckerstoerung in FaWaStatus) then
        begin
          FaWaStatus := [];    // damit alle anderen Fehlermeldungen raus
          FaWaStatus := FaWaStatus + [fwsDruckerstoerung];

          // rufende Funktion beenden
...
procedure TWT60TKmp.DoSimul(ATel: TTel);
...
function TWT60TKmp.ProtGewicht(Beizeichen: string): longint;
var
  Befehl : array[0..200] of AnsiChar;
  BefLen: integer;
  NextS, S1: string;
...
function TWT60TKmp.HoleStatus: longint;
var
  Befehl : array[0..200] of AnsiChar;
  BefLen: integer;
begin
...
function TWT60TKmp.DelSpNr(SpNr: integer): longint;
begin
  Result := -1;
end;

function TWT60TKmp.Quittieren: longint;
begin
  Result := -1;
end;

function TWT60TKmp.Zeilendruck(Zeile: string): longint;
var
  Befehl : array[0..200] of AnsiChar;
  BefLen: integer;
begin
...
function TWT60TKmp.DruckeBlock(ABlock: TStrings): longint;
var
  I: integer;
begin
  Result := -1;
...
procedure TWT60TKmp.UserFnk(ATel: TTel; FnkName : string);
var
  I : integer;
  BccCh : Byte;
...
begin
  ...
  begin
    BccCh := 0;
    for I:= 0 to length(PcName) do
      BccCh := BccCh xor ord(PcName[I]);
    for I:= 0 to length(WaName) do
