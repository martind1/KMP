...
(* SQL Dialog
   ...
   30.05.14 md   Meta: #MetadataKind Schema.Tablename;Indexname
*)
interface
...
uses
  ...
  TgridKmp, Dialogs, DBCtrls, Mugrikmp, qSplitter, UQue_Kmp, UPro_Kmp, UMetaKmp;
...
type
  TDlgSql = class(TqForm)
    ...
    MiMeta: TMenuItem;
    TabSheet5: TTabSheet;
    LbMetaKinds: TListBox;
    LbMetaParams: TListBox;
    ...
    procedure MiMetaClick(Sender: TObject);
  private
    FHistIndex: integer;
  ...
  private
    ...
    procedure ShowDb;
    function MemoToQuery(aSQL: TStrings; NewTab: TTabSheet): boolean;
    procedure MemoToMeta(aLine: string);
...
begin
  ...
  Mu1.OptiWidthFlag := true; //30.05.14
end;
...
function TDlgSql.MemoToQuery(aSQL: TStrings; NewTab: TTabSheet): boolean;
//Öffnet Query, zeigt Ergebnistabelle an, zeigt Parameter an
var
  I: integer;
begin
  Result := true;
  Query1.SQL.Assign(aSQL);
...
procedure TDlgSql.MemoToMeta(aLine: string);
//Konfiguriert und öffnet Metadaten
var
  S1, S2, S3, NextS: string;
begin
  //aLine: #Columns;MyTable oder #Indexes;MyTable
  Meta1.DatabaseName := Query1.DatabaseName;
  S1 := PStrTok(aLine, ';', NextS);
  S2 := PStrTokNext(';', NextS);
  S3 := PStrTokNext(';', NextS);
  Meta1.MetaDataKind := Copy(S1, 2, Maxint);
  {0 Columns
   1 Constraints
   2 IndexColumns
   3 Indexes
   4 MetaDataKinds
   5 ProcedureParameters
   6 Procedures
   7 Restrictions
   8 Tables
   9 Databases
    }
  case LbMetaKinds.Items.IndexOf(Meta1.MetaDataKind) of
    0, 1, 3: Meta1.TableName := S2;
    2: begin
         Meta1.Restrictions.Values['TABLE_NAME'] := '';
         Meta1.Restrictions.Values['INDEX_NAME'] := S2;
       end;
    5: Meta1.Restrictions.Values['PROCEDURE_NAME'] := S2;
    6: Meta1.Restrictions.Values['PROCEDURE_SCHEMA'] := S2;  //Oracle
    8: Meta1.Restrictions.Values['TABLE_SCHEMA'] := S2;  //Oracle
  end;
  DataSource1.Dataset := Meta1;
  Meta1.Open;
  if Meta1.EOF and (Meta1.Tablename <> '') then
  begin
    Meta1.Synonymname := S2;
    Meta1.Open;
  end;
end;

procedure TDlgSql.MiMetaClick(Sender: TObject);
var
  I: integer;
begin
  I := TDlgStrings.Execute(LbMetaKinds.Items, MiMeta.Caption);
  if I >= 0 then
  begin
    SqlMemo.Lines.Text := '#' + LbMetaKinds.Items[I] + ';' + LbMetaParams.Items[I];
  end;
end;
...
begin
  ...
  begin
    ...
    IsMeta := BeginsWith(SqlMemo.Lines.Text, '#');  //ohne CRLF!
    if IsMeta then
    begin
      if NewTab = tsErgebnis then
      try
        MemoToMeta(SqlMemo.Lines[0]);
    ...
    begin
      if not MemoToQuery(SqlMemo.Lines, NewTab) then
        TPageControl(Sender).ActivePage := OldTab;
    end;
  end;
  if NewTab <> tsErgebnis then
