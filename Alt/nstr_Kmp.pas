...
begin
  ...
  try
    ...
    begin
      ...
      begin
        D.Add(Format('%04.4d', [K2A]));
        if IsProt then
          ProtA('D%d(%s)', [D.Count - 1, D[D.Count - 1]]);
        D.Add(Format('%04.4d', [K2E]));
        if IsProt then
          ProtA('D%d(%s)', [D.Count - 1, D[D.Count - 1]]);
      end;
    end;
...
begin
  case DataType of
    ...
    ftDate: begin
      (*if CompareText(Opn, 'SysDate') = 0 then
        DateOpn := date else
        DateOpn := StrToDate4(Opn);               { Check ob korrektes Datum }*)
      if PosI('sysdate', Opn) > 0 then
      begin
        DateOpn := date;   //todo: Arithmetik z.B. sysdate - 365
      end else
        DateOpn := StrToDate4(Opn);               { Check ob korrektes Datum }
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          AddStr(SQL, Format('((%s >= %s) and (%s < %s))',
            [Col, Opn1, Col, OpnDt2]));
          done := true;
        end else
        if (ColumnType in [ftSmallint, ftInteger, ftAutoInc, ftWord,
        ...
        begin                                          {9*** -> 9000..9999}
          ...
          AddStr(SQL, Format('((%s >= %s) and (%s <= %s))', [Col, Opn1, Col, Opn2]));
          done := true;
...
{$endif}
    ...
    if not done then
    begin
      AddStr(SQL, Format('(%s %s %s)', [Col, Opt, Opn1]));
      {'(' + Col + ' ' + Opt + ' ' + Opn1 + ')');}
    end;
  end;
var
